...
%setting for the plots line size
widthline=2;

%reference to be plotted
reference_to_plot=zeros(size(C,1),round(Tsim/Ts));
...
for i=1:length(time_to_plot)
    ...
    r_curr=reference_value_vec(:,index);
    reference_to_plot(:,i)=r_curr;
end

% the following plots the sensor measurement y obtained from the simulation in Simulink 
figure
...
grid on
title(sprintf('Output %i\n',i))
xlabel('time [sec]')
ylabel(sprintf('y%i',i))
legend('Output','Reference')
end

% the following plots the control signal u obtained from the simulation in Simulink 
figure
...
for i=1:size(u_seq,2)
    ...
    grid on
    xlabel('time [sec]')
    title(sprintf('Control Input %i\n',i))
    xlabel('time [sec]')
    ylabel(sprintf('u%i',i))
end

% real state vs state estimator
figure
...
% adjust the font for all the figures
