...
file_simulink = 'NetworkedControlSystemBasic_Part4.slx';
...
fprintf('\nRunning %d trials under sensor attack...\n', num_trials);

for trial = 1:num_trials
    output = sim(file_simulink);

    y_seq = output.y.signals.values;
    result_seq = output.detector_outcome_2.signals.values;
    ...
    % Index ranges for attack and non-attack periods
    attack_idx = find(time_seq >= 240 & time_seq <= 280);
    normal_idx = find(time_seq < 240);

    % Count detection results
    TP = TP + sum(result_seq(attack_idx) == 1);
    FN = FN + sum(result_seq(attack_idx) == 0);
    FP = FP + sum(result_seq(normal_idx) == 1);
    TN = TN + sum(result_seq(normal_idx) == 0);

    % Compute Je(0, 200)
    idx_je = find(time_seq <= 200);  % indices where t in [0, 200]
    Je_total = Je_total + sum((y_seq(idx_je) - ref_seq(idx_je)).^2) / length(idx_je);
end
...
% Display results
fprintf('\n--- PART 4 RESULTS (Replay Attack 1.2 with Watermarking) ---\n');
...
results = table(TP, FN, FP, TN, drate, falarm, F1, Je_avg, ...
    'VariableNames', {'TP','FN','FP','TN','DetectionRate','FalseAlarmRate','F1_Score','Je'});
disp(results);
...
% Plot 3: Real vs Estimated State
figure
...
xlabel('Time step k')
ylabel('z(k)')
title(sprintf('Chi-squared Test Statistic with Watermarking (\\tau = %.4f)', tau))
legend('z(k)', sprintf('\\tau = %.4f', tau))
grid on

% Plot 5: Detector Output Over Time
figure
result_seq = output.detector_outcome_2.signals.values;  % already logged
time_seq = output.detector_outcome_2.time;

stairs(time_seq, result_seq, 'b', 'LineWidth', widthline)
ylim([-0.2 1.2])
yticks([0 1])
yticklabels({'No Attack', 'Attack'})
xlabel('Time [sec]')
ylabel('Detector Output')
title('Detector Output Over Time')
grid on

% Global font and style for all figures
